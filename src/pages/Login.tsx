...
const Login = () => {
  ...
  const handleSubmit = async (values: { email: string; password: string }, { setSubmitting }: { setSubmitting: (submitting: boolean) => void }) => {
    setLoading(true);
    setError('');
    try {
      ...
      setError((err as Error).message || t('login.errors.loginError'));
    } finally {
      setLoading(false);
      setSubmitting(false);
    }
