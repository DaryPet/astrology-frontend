...
const ResetPassword = () => {
  ...
  const validationSchema = Yup.object({
    ...
    confirmPassword: Yup.string()
      .oneOf([Yup.ref('password'), undefined], t('resetPassword.validation.passwordsMismatch'))
      .required(t('resetPassword.validation.confirmRequired'))
  });

  const handleSubmit = async (values: { password: string; confirmPassword: string }, { setSubmitting }: { setSubmitting: (submitting: boolean) => void }) => {
    setLoading(true);
    setError('');
    try {
      ...
      setError((err as Error).message || t('resetPassword.errors.updateError'));
    } finally {
      setLoading(false);
      setSubmitting(false);
    }
