...
const Register = () => {
  ...
  const validationSchema = Yup.object({
    ...
    confirmPassword: Yup.string()
      .oneOf([Yup.ref('password'), undefined], t('register.validation.passwordsMismatch'))
      .required(t('register.validation.confirmRequired'))
  });

  const handleSubmit = async (values: { name: string; email: string; password: string; confirmPassword: string }, { setSubmitting }: { setSubmitting: (submitting: boolean) => void }) => {
    setLoading(true);
    setError('');
    try {
      ...
      setError((err as Error).message || t('register.errors.registerError'));
    } finally {
      setLoading(false);
      setSubmitting(false);
    }
