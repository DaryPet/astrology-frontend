...
function Home() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Backend САМ определит координаты по названию города!
      const chartData = {
      ...
      console.log('📤 Отправляю в backend (backend сам определит координаты):', chartData)
      ...
      let errorMessage = 'Ошибка при расчете карты'
      
      // Извлекаем сообщение из структурированной ошибки FastAPI
      if (errorData) {
        ...
        errorMessage = err.message || 'Ошибка при расчете карты'
      }
      
      // Особенная обработка ошибок
      if (errorMessage.includes('Cannot determine coordinates') || 
        ...
        setError(`Ошибка расчета карты. Проверьте что:
        1. Город выбран из списка автокомплита
        2. Координаты определены (${coordinates.lat ? '✓' : '✗'})
        3. Дата рождения указана правильно
            ...
            <form onSubmit={handleSubmit}>
              ...
              {(!coordinates.lat || !formData.birth_date) && (
                <div style={{
                  ...
                  {!formData.birth_date ? 'Укажите дату рождения' : 
                   !coordinates.lat ? 'Выберите город из списка' : 
                   'Готово к расчету!'}
                  {coordinates.lat && (
                    <div style={{ marginTop: '4px', fontSize: '11px', color: 'var(--text-secondary)' }}>
                      Координаты: {coordinates.lat.toFixed(4)}°, {coordinates.lon.toFixed(4)}°
                    </div>
                  )}
                </div>
              )}
