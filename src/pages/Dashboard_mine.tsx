...
//         try {
//           const dbMessages = await chartsApi.getChatMessages(Number(chart.id));
//           if (dbMessages.length > 0) {
//             setChatHistory(dbMessages as ChatMessage[]);
//           }
//         } catch (err) {
...
//     const chartIdAtSendTime = savedChartId; // Capture chartId at send time to prevent race conditions
//     const userMessage = { role: 'user' as const, content: questionText };
//     const updatedWithUser = [...chatHistory, userMessage];
//     setChatHistory(updatedWithUser);
//     // Save to database immediately
//     chartsApi.saveChatMessages(Number(chartIdAtSendTime), updatedWithUser).catch(err => {
...
//       };
//       // Always save to DB for the correct chat (chartIdAtSendTime)
//       const updated = [...chatHistory, botMessage];
//       chartsApi.saveChatMessages(Number(chartIdAtSendTime), updated).catch(err => {
//         console.error('Failed to save chat history:', err);
//       });
//       // Only update UI if still on the same chart
//       if (savedChartId === chartIdAtSendTime) {
//         setChatHistory(updated);
//       }
//     } catch (error) {
//       // Only show error if still on the same chart
//       if (savedChartId === chartIdAtSendTime) {
...
//         try {
//           await chartsApi.saveChatMessages(Number(saved.id), chatHistory);
//         } catch (err) {
//           console.error('Failed to save chat history:', err);
//         }
//       }
